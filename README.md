...
- `generate/build/run` **не пересоздают** `build/` каждый раз, а обрабатывают только изменившиеся файлы.
- Для полного пересоздания используйте флаг `--clean`.
...
Полная пересборка (без инкремента):

```bash
godsl generate --clean <путь проекта>
godsl build --clean <путь проекта>
godsl run --clean <путь проекта>
```
